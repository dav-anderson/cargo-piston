...
impl LinuxBuilder {
    ...
    fn new(
        ...
        println!(
            "creating LinuxBuilder: release: {:?}, target: {:?}, cwd: {:?}",
            release,
            target.to_string(),
            cwd
        );
        ...
        if std::env::consts::OS == "macos" {
            ...
            zigbuild_path = Some(env_vars.get("zigbuild_path").cloned().ok_or(
                PistonError::ZigbuildMissingError("Zigbuild key not found".to_string()),
            )?);
            println!(
                "Zigbuild path determined: {}",
                &zigbuild_path.clone().unwrap()
            );
            homebrew_path = Some(env_vars.get("homebrew_path").cloned().ok_or(
                PistonError::HomebrewMissingError("Homebrew key not found".to_string()),
            )?);
            println!(
                "Homebrew path determined: {}",
                &homebrew_path.clone().unwrap()
            );
        }
    ...
    fn build(&mut self) -> Result<(), PistonError> {
        ...
        if std::env::consts::OS == "macos" {
            ...
            let builder = Command::new("bash")
                ...
                .arg(format!(
                    "{} {}",
                    self.zigbuild_path.as_ref().unwrap(),
                    &cargo_args
                ))
    ...
    fn post_build(&mut self) -> Result<(), PistonError> {
        ...
        let assets: Option<PathBuf> = if assets_path.exists() {
            Some(assets_path)
        } else {
            None
        };
        ...
        if self.appimage {
            if !self.runtime_path.is_none() {
                ...
                return Err(PistonError::Generic(format!(
                    "Missing path to runtimes in .env"
                )));
            }
        //static binary
        } else {
...
impl LinuxRunner {
    ...
    fn new(
        ...
        println!(
            "Creating Linux Runner: release flag: {:?}, cwd: {:?}",
            release, cwd
        );
        //parse env vars
...
impl GPGSigner {
    ...
    fn gpg_sign(
        ...
        if let Some(pass) = key_pass {
            if let Some(mut stdin) = child.stdin.take() {
                match stdin.write(pass.as_bytes()) {
                    Ok(res) => res,
                    Err(..) => {
                        return "Error writing passphrase to bytes, GPG signing failed".to_string();
                    }
                };
            }
        }
        ...
        return format!(
            "successfully signed {} with signature at {:?}",
            bundle_path.display(),
            sig_path.display()
        );
    }
    ...
    pub fn build(
        ...
        .map_err(|e| {
            PistonError::Generic(format!(
                "Failed to create dir usr/share/applications: {}",
                e
            ))
        })?;
        ...
        .map_err(|e| {
            PistonError::Generic(format!("Failed to create dir usr/share/icons/...: {}", e))
        })?;
        ...
        // Binary
        {
            ...
            .map_err(|e| {
                PistonError::Generic(format!("Failed to push binary into AppDir: {}", e))
            })?;
        }

        // AppRun
        let apprun_content = format!("#!/bin/sh\nexec \"$APPDIR/usr/bin/{}\" \"$@\"\n", app_name);
        fs.push_file(
...
"#,
        ...
        {
            let file = fs::read(&icon_path).map_err(|e| {
                PistonError::Generic(format!(
                    "Failed to read icon {}: {}",
                    icon_path.display(),
                    e
            ...
            fs.push_file(Cursor::new(file), &icon_dest, header)
                .map_err(|e| {
                    PistonError::Generic(format!("Failed to push icon into AppDir: {}", e))
                })?;
        }
        ...
        if let Some(assets) = &assets {
            ...
            .map_err(|e| {
                PistonError::Generic(format!("Failed to create assets directory: {}", e))
            })?;
        ...
        let runtime_filename = match target.as_str() {
            ...
            t if t.starts_with("armv7") || t.starts_with("arm-") || t.starts_with("armv6") => {
                "runtime-armhf"
            }
            t if t.starts_with("i686") || t.starts_with("i386") => "runtime-i686",
