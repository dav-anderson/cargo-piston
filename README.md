...
Universal binaries are automatically created via lipo when you pass in the release flag to a macos target triple build command. For example running the following command will automatically create a universal binary in the output bundle that will run on either arm64 or x86_64 architecture.
