...
Any app that is compiled with the `--release` flag will automatically be signed for apple app store distribution, provided automated signing is properly configured.
...
Note: This step is only necessary if you do not have an admin level AppStoreConnect API key or if you wish to distribute apps outside of the Apple App Store ecosystem. The name of the key automatically generated by cargo-piston will be `piston.pem`.
